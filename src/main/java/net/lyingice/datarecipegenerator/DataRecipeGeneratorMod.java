package net.lyingice.datarecipegenerator;
...
@Mod("data_recipe_generator")
public class DataRecipeGeneratorMod {
	public static final Logger LOGGER = LogManager.getLogger(DataRecipeGeneratorMod.class);
	public static final String MODID = "data_recipe_generator";

	public DataRecipeGeneratorMod(IEventBus modEventBus) {
		// Start of user code block mod constructor
		// End of user code block mod constructor
		NeoForge.EVENT_BUS.register(this);
		modEventBus.addListener(this::registerNetworking);
		DataRecipeGeneratorModMenus.REGISTRY.register(modEventBus);
        addServerBoundNetworkMessage(
                GenerateRecipeMessage.TYPE,
                GenerateRecipeMessage.STREAM_CODEC,
                GenerateRecipeMessage::handleGenerate
        );
        addNetworkMessage(
	...
	private static final Set<CustomPacketPayload.Type<?>> SERVER_BOUND_MESSAGES = new HashSet<>();
	...
	public static <T extends CustomPacketPayload> void addServerBoundNetworkMessage(CustomPacketPayload.Type<T> id, StreamCodec<? extends FriendlyByteBuf, T> reader, IPayloadHandler<T> handler) {
		if (networkingRegistered)
			throw new IllegalStateException("Cannot register new network messages after networking has been registered");
		MESSAGES.put(id, new NetworkMessage<>(reader, handler));
		SERVER_BOUND_MESSAGES.add(id);
	}
	...
	private void registerNetworking(final RegisterPayloadHandlersEvent event) {
		...
		MESSAGES.forEach((id, networkMessage) -> {
			if (SERVER_BOUND_MESSAGES.contains(id)) {
				registrar.playToServer(id, ((NetworkMessage) networkMessage).reader(), ((NetworkMessage) networkMessage).handler());
			} else {
